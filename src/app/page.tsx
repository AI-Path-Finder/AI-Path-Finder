...
export default function LandingPage() {
  return (
    <PageTransition>
          ...
          <div className="mx-auto max-w-[1440px]">
            <div className="mx-auto max-w-4xl text-center">
              ...
              <p className="mx-auto mt-7 max-w-2xl text-lg leading-relaxed text-muted-foreground">
                ProcessMind asks the right questions, evaluates every opportunity,
                and makes the business case easy to understand.
              </p>
            </div>
            <div className="mt-20 grid gap-5 lg:grid-cols-3">
              <ColorPanel color="bg-[#e8ddff]" label="DISCOVER" title="Describe your company naturally" text="A conversational assessment learns how teams work, where time is lost, and what slows people down." />
              <ColorPanel color="bg-[#ffdd8d]" label="PRIORITIZE" title="Know what should happen first" text="Every initiative is scored for value, feasibility, speed, risk, and data readiness." />
              <ColorPanel color="bg-[#ff9d86]" label="PROVE" title="Make the financial case" text="Model savings, implementation cost, payback period, and twelve-month return." />
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-[1480px] rounded-[40px] bg-[#f0edff] px-6 py-20 md:rounded-[68px] md:px-16 md:py-28">
            <div className="grid gap-16 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
              ...
              <div className="rounded-[32px] bg-white p-5 shadow-[0_24px_70px_rgba(47,28,77,.14)] md:p-9">
                <div className="mb-12 flex items-center justify-between text-xs text-muted-foreground"><span>ProcessMind discovery</span><span>3 of 7</span></div>
      ...
      <footer className="px-6 py-10 md:px-10"><div className="mx-auto flex max-w-[1440px] justify-between text-sm"><strong>ProcessMind</strong><span className="text-muted-foreground">AI opportunity intelligence</span></div></footer>
    </PageTransition>
  );
}
  ...
  return (
    <motion.div initial={{ opacity: 0, rotate: 2, y: 20 }} animate={{ opacity: 1, rotate: -2, y: 0 }} transition={{ delay: .15, duration: .5 }} className="relative mx-auto w-full max-w-xl">
      ...
      <div className="relative rounded-[28px] bg-[#f8f5ff] p-6 text-[#2f1c4d] shadow-[0_28px_80px_rgba(0,0,0,.28)] md:p-9">
        <div className="mb-16 flex justify-between text-xs text-[#2f1c4d]/55"><span>ProcessMind</span><span>1 of 7</span></div>
        ...
        <div className="mt-8 space-y-2">
          {["Technology", "Financial services", "Healthcare"].map((label, i) => <div key={label} className={`flex items-center gap-3 rounded-lg border-2 px-4 py-3 text-sm font-medium ${i === 0 ? "border-[#6b4eff] bg-[#e8ddff]" : "border-[#2f1c4d]/15"}`}><span className="flex h-6 w-6 items-center justify-center rounded border border-current text-xs">{i + 1}</span>{label}</div>)}
        </div>
        <Button className="mt-7 bg-[#6b4eff] text-white hover:bg-[#5438de]">Continue <ArrowRight /></Button>
      </div>
    </motion.div>
  );
}

function ColorPanel({ color, label, title, text }: { color: string; label: string; title: string; text: string }) {
  return <article className={`${color} flex min-h-[430px] flex-col justify-between rounded-[34px] p-8 md:p-10`}><span className="text-sm font-semibold">{label}</span><div><h3 className="text-4xl font-semibold leading-[1.04] tracking-[-.055em]">{title}</h3><p className="mt-5 leading-relaxed text-foreground/65">{text}</p></div></article>;
}
